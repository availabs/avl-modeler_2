...
const VarSelector = ({ projectData, varList }) => {
  ...
  useEffect(() => {
    const getVariables = async () => {
      ...
      console.log("modelVariableData--", data);

      setVariables(data);

      setModelVars(Object.keys(data).filter((key) => data[key].required));
      //   setModelVars(Object.keys(data).filter((key) => data[key]));
    };
    getVariables();
  }, []);
  ...
  console.log("variables---", variables, modelVars, newVariables, varList);
  ...
  return (
    <div>
      <div className="mt-1 mb-2 ml-auto px-2 py-2">
        ...
        <Charts data={projectData} selectedVar={modelVars} />
      </div>
    </div>
  );
};
