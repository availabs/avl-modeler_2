...
// checking the ExifPhoto table for sanity 
  ...
  ok_photoIdPk = false;
  err_pk = false;
  xsql = (char *) "PRAGMA table_info(ExifPhoto)";
  ret =
...
// checking the ExifTags table for sanity 
  ...
  ok_tagIdPk = false;
  err_pk = false;
  xsql = (char *) "PRAGMA table_info(ExifTags)";
  ret =
    sqlite3_get_table(SqliteHandle, xsql, &results, &rows, &columns, &errMsg);
  if (ret != SQLITE_OK)
    {
...
// checking the ExifValues table for sanity 
  ...
  err_pk = false;
  xsql = (char *) "PRAGMA table_info(ExifValues)";
  ret =
    sqlite3_get_table(SqliteHandle, xsql, &results, &rows, &columns, &errMsg);
  if (ret != SQLITE_OK)
    {
...
// feeding the ExifPhoto table; preparing the SQL statement
  ...
  if (ok == true)
    {
      char *dummy = (char *) malloc((make.Len() * 4) + 1);
  ...
  if (ok == true)
    {
      char *dummy = (char *) malloc((model.Len() * 4) + 1);
  ...
  if (ok == true)
    {
      char *dummy = (char *) malloc((date.Len() * 4) + 1);
  ...
  if (ok == true)
    {
      char *dummy = (char *) malloc((satellites.Len() * 4) + 1);
  ...
  if (ok == true)
    {
      char *dummy = (char *) malloc((timestamp.Len() * 4) + 1);
  ...
  if (path.Len() > 0)
    {
      char *dummy = (char *) malloc((path.Len() * 4) + 1);
...
void MyFrame::ImportGpsPhotos(wxString & path, bool folder, wxString & table,
                              wxString & geometry, bool update_statistics,
                              bool spatial_index)
{
//
// trying to import EXIF-GPS photos
// 
  int ret;
  char *sql;
...
// starts a transaction 
  ...
  if (CheckGpsPicsTable(table, geometry, spatial_index) == false)
    {
      ::wxEndBusyCursor();
      wxMessageBox(wxT
                   ("A table is already defined with the same name, but it has incompatibles columns"),
                   wxT("spatialite_gui"), wxOK | wxICON_ERROR, this);
      goto abort;
    }
// feeding the ExifPhoto table; preparing the SQL statement
  ...
  qgeom = gaiaDoubleQuotedSql(xgeom);
  sql =
    sqlite3_mprintf
    ("INSERT INTO \"%s\" (id, name, width, height, photo, timestamp, \"%s\") "
     "VALUES (NULL, ?, ?, ?, ?, ?, ?)", qtable, qgeom);
  free(qtable);
  free(qgeom);
  ret = sqlite3_prepare_v2(SqliteHandle, sql, strlen(sql), &stmt, NULL);
  sqlite3_free(sql);
  if (ret != SQLITE_OK)
    {
      wxMessageBox(wxT("INSERT INTO GpsPicsTable error: ") +
                   wxString::FromUTF8(sqlite3_errmsg(SqliteHandle)),
                   wxT("spatialite_gui"), wxOK | wxICON_ERROR, this);
      goto abort;
    }
  ...
  if (folder == true)
    cnt = GpsPicsLoadDir(path, stmt);
  else
    cnt = GpsPicsLoadFile(path, stmt);
  sqlite3_finalize(stmt);
...
// commits the transaction 
  ...
  if (update_statistics == true)
    {
      // immediately updating Layer Statistics
      sql = sqlite3_mprintf("SELECT UpdateLayerStatistics(%Q)", xtable);
      ret = sqlite3_exec(SqliteHandle, sql, NULL, NULL, &errMsg);
      sqlite3_free(sql);
      if (ret != SQLITE_OK)
        {
...
// rolling back the transaction
  if (stmt != NULL)
    sqlite3_finalize(stmt);
  sqlite3_exec(SqliteHandle, "ROLLBACK", NULL, NULL, &errMsg);
...
bool MyFrame::CheckGpsPicsTable(wxString & table, wxString & geometry,
                                bool rtree)
{
//
// creates a GpsPics table / or checks existing ones for validity 
//
  int ret;
  char *sql;
  char *errMsg;
...
// testing if the table already exists 
  strcpy(xtable, table.ToUTF8());
  strcpy(xgeom, geometry.ToUTF8());
  qtable = gaiaDoubleQuotedSql(xtable);
  qgeom = gaiaDoubleQuotedSql(xgeom);
  sql = sqlite3_mprintf("PRAGMA table_info(\"%s\")", qtable);
  ret =
    sqlite3_get_table(SqliteHandle, sql, &results, &rows, &columns, &errMsg);
  sqlite3_free(sql);
  if (ret != SQLITE_OK)
    {
      wxMessageBox(wxT("PRAGMA table_info(GpsPica-table) error: ") +
                   wxString::FromUTF8(errMsg), wxT("spatialite_gui"),
                   wxOK | wxICON_ERROR, this);
      sqlite3_free(errMsg);
      goto abort;
      ...
      for (i = 1; i <= rows; i++)
        {
          ...
          if (ok_photo || ok_width || ok_height || ok_photo || ok_timestamp)
            continue;
          if (atoi(name) != 0)
            {
              err = true;
...
// testing for Geometry
  sql =
    sqlite3_mprintf
    ("SELECT count(*) FROM geometry_columns WHERE f_table_name = Lower(%Q) "
     "AND f_geometry_column = Lower(%Q) AND srid = 4326 AND "
     "geometry_type = 1 AND coord_dimension = 2", xtable, xgeom);
  ret =
    sqlite3_get_table(SqliteHandle, sql, &results, &rows, &columns, &errMsg);
  sqlite3_free(sql);
  if (ret != SQLITE_OK)
    {
      wxMessageBox(wxT("Check Geometry GpsPica-table error: ") +
                   wxString::FromUTF8(errMsg), wxT("spatialite_gui"),
                   wxOK | wxICON_ERROR, this);
      sqlite3_free(errMsg);
      goto abort;
...
create_new:
// creating the GpsPics table 
  sql = sqlite3_mprintf("CREATE TABLE \"%s\" ("
                        "\tid INTEGER PRIMARY KEY AUTOINCREMENT,\n"
                        "\tname TEXT NOT NULL,\n"
                        "\theight INTEGER NOT NULL,\n"
                        "\twidth INTEGER NOT NULL,\n"
                        "\tphoto BLOB NOT NULL,\n" "\ttimestamp TEXT)", qtable);
  ret = sqlite3_exec(SqliteHandle, sql, NULL, NULL, &errMsg);
  sqlite3_free(sql);
  if (ret != SQLITE_OK)
    {
      wxMessageBox(wxT("CREATE TABLE GpsPicsTable error: ") +
                   wxString::FromUTF8(errMsg), wxT("spatialite_gui"),
                   wxOK | wxICON_ERROR, this);
      sqlite3_free(errMsg);
      goto abort;
...
// creating the Geometry Column
  sql =
    sqlite3_mprintf("SELECT AddGeometryColumn(%Q, %Q, 4326, 'POINT', 'XY')",
                    xtable, xgeom);
  ret = sqlite3_exec(SqliteHandle, sql, NULL, NULL, &errMsg);
  sqlite3_free(sql);
  if (ret != SQLITE_OK)
    {
      wxMessageBox(wxT("ADD GEOMETRY COLUMN GpsPicsTable error: ") +
                   wxString::FromUTF8(errMsg), wxT("spatialite_gui"),
                   wxOK | wxICON_ERROR, this);
      sqlite3_free(errMsg);
      goto abort;
  ...
  if (rtree)
    {
      ...
      sql = sqlite3_mprintf("SELECT CreateSpatialIndex(%Q, %Q)", xtable, xgeom);
      ret = sqlite3_exec(SqliteHandle, sql, NULL, NULL, &errMsg);
      sqlite3_free(sql);
      if (ret != SQLITE_OK)
        {
          wxMessageBox(wxT("CREATE SPATIAL INDEX GpsPicsTable error: ") +
                       wxString::FromUTF8(errMsg), wxT("spatialite_gui"),
                       wxOK | wxICON_ERROR, this);
          sqlite3_free(errMsg);
          goto abort;
...
int MyFrame::GpsPicsLoadDir(wxString & path, sqlite3_stmt * stmt)
{
//
// importing EXIF-GPS files from a whole DIRECTORY 
//
#if defined(_WIN32) && !defined(__MINGW32__)
// Visual Studio .NET
  struct _finddata_t c_file;
      ...
      while (1)
        {
          if ((c_file.attrib & _A_RDONLY) == _A_RDONLY
              || (c_file.attrib & _A_NORMAL) == _A_NORMAL)
            {
              filePath = path;
              filePath += wxT("/") + wxString::FromUTF8(c_file.name);
              cnt += GpsPicsLoadFile(filePath, stmt);
            }
          if (_findnext(hFile, &c_file) != 0)
            break;
        };
...
int MyFrame::GpsPicsLoadFile(wxString & path, sqlite3_stmt * stmt)
{
//
// importing a single EXIF-GPS file 
//
  FILE *fl;
  int sz = 0;
  int rd;
      ...
      if (rd == sz)
        {
          ...
          if (tag_list)
            {
              if (IsExifGps(tag_list) == false)
                goto stop;
              if (ImportExifGps(blob, sz, tag_list, name, stmt) == false)
                goto stop;
              loaded = 1;
            }
        }
...
bool MyFrame::ImportExifGps(unsigned char *blob, int sz,
                            gaiaExifTagListPtr tag_list, wxString & path,
                            sqlite3_stmt * stmt)
{
//
// importing a single EXIF-GPS image into the DB Table
//
  sqlite3_int64 val64;
  int ret;
  wxString timestamp;
  ...
  if (ok == true)
    ...
    goto abort;
  sqlite3_bind_blob(stmt, 4, blob, sz, SQLITE_STATIC);
  GetGpsTimestamp(tag_list, timestamp, &ok);
  if (ok == true)
    {
      char *dummy = (char *) malloc((timestamp.Len() * 4) + 1);
  ...
  if (ok == true)
    {
      ...
      gaiaToSpatiaLiteBlobWkb(geom, &geoblob, &geosize);
      gaiaFreeGeomColl(geom);
      sqlite3_bind_blob(stmt, 6, geoblob, geosize, SQLITE_TRANSIENT);
      free(geoblob);
  } else
    goto abort;
  ret = sqlite3_step(stmt);
