...
//
  ...
  else if (BlobType == GAIA_COMPRESSED_GEOMETRY_BLOB)
    Geometry = gaiaFromSpatiaLiteBlobWkb(Blob, BlobSize);
            ...
            if (ret == RL2_OK)
              {
                ...
                rl2GraphicsContextPtr ctx =
                  rl2_graph_create_context(MainFrame->GetRL2PrivateData(), 560,
                                           290);
...
// creates individual panels
  ...
  if (BlobType == GAIA_GEOMETRY_BLOB
      || BlobType == GAIA_COMPRESSED_GEOMETRY_BLOB
      || BlobType == GAIA_TINYPOINT_BLOB || BlobType == GAIA_GPB_BLOB)
    {
