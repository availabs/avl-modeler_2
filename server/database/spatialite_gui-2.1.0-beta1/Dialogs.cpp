...
bool VirtualShpDialog::Create(MyFrame * parent, wxString & path,
                              wxString & table, int srid, wxString & defCs)
{
//
// creating the dialog
//
...
// third row: CHARSET
  ...
  if (idSel != wxNOT_FOUND)
    charsetCtrl->SetSelection(idSel);
  charsetSizer->Add(charsetCtrl, 0, wxALIGN_CENTER_VERTICAL | wxALL, 0);
// column-names Lower/Uppercase
  wxBoxSizer *caseBox = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(caseBox, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxString case_selection[3];
  case_selection[0] = wxT("convert to Lowercase");
  case_selection[1] = wxT("convert to Uppercase");
  case_selection[2] = wxT("never change");
  wxRadioBox *case_sel = new wxRadioBox(this, ID_LDDBF_COLNAME_CASE,
                                        wxT("&Column Names"),
                                        wxDefaultPosition,
                                        wxDefaultSize, 3,
                                        case_selection, 3,
                                        wxRA_SPECIFY_COLS);
  case_sel->SetSelection(0);
  caseBox->Add(case_sel, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 1);
...
//
  ...
  wxRadioBox *case_sel = (wxRadioBox *) FindWindow(ID_LDDBF_COLNAME_CASE);
  if (case_sel->GetSelection() == 0)
    ColnameCase = GAIA_DBF_COLNAME_LOWERCASE;
  else if (case_sel->GetSelection() == 1)
    ColnameCase = GAIA_DBF_COLNAME_UPPERCASE;
  else
    ColnameCase = GAIA_DBF_COLNAME_CASE_IGNORE;
  wxDialog::EndModal(wxID_OK);
}

LoadZipDbfDialog::~LoadZipDbfDialog()
{
// destructor
  if (PKFields != NULL)
    delete[]PKFields;
...
bool LoadZipDbfDialog::Create(MyFrame * parent, wxString & zip_path,
                              wxString & filename, wxString & table,
                              wxString & defCs)
{
//
// creating the dialog
//
  ...
  Default = defCs;
  LoadPKFields();
  if (wxDialog::Create(parent, wxID_ANY, wxT("Load DBF file (from Zipfile)")) ==
      false)
    return false;
// populates individual controls
  CreateControls();
...
void LoadZipDbfDialog::CreateControls()
{
//
// creating individual control and setting initial values
//
...
// first row: the Shapefile path
  ...
  boxSizer->Add(box2Sizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 1);
  wxBoxSizer *pathSizer = new wxBoxSizer(wxHORIZONTAL);
  box2Sizer->Add(pathSizer, 0, wxALIGN_RIGHT | wxALL, 0);
  wxStaticText *pathLabel =
    new wxStaticText(this, wxID_STATIC, wxT("Zipfile &Path:"));
  pathSizer->Add(pathLabel, 0, wxALIGN_CENTER_VERTICAL | wxALL, 1);
  wxTextCtrl *pathValue = new wxTextCtrl(this, wxID_STATIC,
                                         ZipPath, wxDefaultPosition,
                                         wxSize(350, 22), wxTE_READONLY);
  pathSizer->Add(pathValue, 0, wxALIGN_RIGHT | wxALL, 1);
  wxBoxSizer *fileSizer = new wxBoxSizer(wxHORIZONTAL);
  box2Sizer->Add(fileSizer, 0, wxALIGN_RIGHT | wxALL, 0);
  wxStaticText *fileLabel =
    new wxStaticText(this, wxID_STATIC, wxT("DBF file:"));
  fileSizer->Add(fileLabel, 0, wxALIGN_CENTER_VERTICAL | wxALL, 1);
  wxTextCtrl *fileValue = new wxTextCtrl(this, wxID_STATIC,
                                         Filename, wxDefaultPosition,
                                         wxSize(350, 22), wxTE_READONLY);
  fileSizer->Add(fileValue, 0, wxALIGN_RIGHT | wxALL, 1);
// second row: TABLE name
  wxBoxSizer *tableSizer = new wxBoxSizer(wxHORIZONTAL);
  box2Sizer->Add(tableSizer, 0, wxALIGN_RIGHT | wxALL, 0);
  wxStaticText *tableLabel =
    new wxStaticText(this, wxID_STATIC, wxT("&Table name:"));
...
// appends event handler for OK button
  Connect(wxID_OK, wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction) & LoadZipDbfDialog::OnOk);
  Connect(ID_LDDBF_USER_PKEY, wxEVT_COMMAND_RADIOBOX_SELECTED,
          (wxObjectEventFunction) & LoadZipDbfDialog::OnUserPKey);
  Connect(ID_LDDBF_TEXTDATES, wxEVT_COMMAND_RADIOBOX_SELECTED,
          (wxObjectEventFunction) & LoadZipDbfDialog::OnTextDates);
}

void LoadZipDbfDialog::LoadPKFields()
{
// loading the Primary Key Fields list
  char *zip_path;
  char *filename;
...
// opening the DBF in order to fetch field names 
  cs = *(charsets + idSel);
  strcpy(charset, cs.ToUTF8());
  zip_path = (char *) malloc(ZipPath.Len() * 4);
...
void LoadZipDbfDialog::OnUserPKey(wxCommandEvent & WXUNUSED(event))
{
//
// the mode of user defined Primary Key Column changed
//
...
void LoadZipDbfDialog::OnTextDates(wxCommandEvent & WXUNUSED(event))
{
//
// the mode of user defined DBF TextDates changed
//
...
void LoadZipDbfDialog::OnOk(wxCommandEvent & WXUNUSED(event))
{
//
// all done: 
//
...
// third row: GEOMETRY COLUMN name
  ...
  box2Sizer->Add(colSizer, 0, wxALIGN_RIGHT | wxALL, 0);
...
// fourth row: SRID
  wxBoxSizer *mixHSizer = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(mixHSizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxBoxSizer *mixVSizer = new wxBoxSizer(wxVERTICAL);
  mixHSizer->Add(mixVSizer, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
  wxBoxSizer *sridSizer = new wxBoxSizer(wxHORIZONTAL);
  mixVSizer->Add(sridSizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxStaticText *sridLabel = new wxStaticText(this, wxID_STATIC, wxT("&SRID:"));
  ...
  mixVSizer->AddSpacer(10);
// DBF dates interpreatation
  wxBoxSizer *dbfDatesBox = new wxBoxSizer(wxHORIZONTAL);
  mixVSizer->Add(dbfDatesBox, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxString dbf_dates[2];
  dbf_dates[0] = wxT("as Julian Day numbers");
  dbf_dates[1] = wxT("as PlainText strings");
  ...
  mixVSizer->AddSpacer(10);
// UpdateStatistics
  wxBoxSizer *statisticsBox = new wxBoxSizer(wxHORIZONTAL);
  mixVSizer->Add(statisticsBox, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxStaticBox *statBox = new wxStaticBox(this, wxID_STATIC,
                                         wxT("Layer Statistics"),
                                         wxDefaultPosition,
                                         wxDefaultSize);
  wxBoxSizer *statSizer = new wxStaticBoxSizer(statBox, wxVERTICAL);
  statisticsBox->Add(statSizer, 0, wxALIGN_LEFT | wxALL, 1);
  wxCheckBox *statisticsCtrl = new wxCheckBox(this, ID_LDSHP_STATISTICS,
                                              wxT("Update immediately"),
                                              wxDefaultPosition, wxDefaultSize);
  statisticsCtrl->SetValue(UpdateStatistics);
  statSizer->Add(statisticsCtrl, 0, wxALIGN_LEFT | wxALL, 1);
// fifth row: CHARSET
  wxStaticBox *charsetBox = new wxStaticBox(this, wxID_STATIC,
                                            wxT("Charset Encoding"),
                                            wxDefaultPosition,
                                            wxDefaultSize);
  wxBoxSizer *charsetSizer = new wxStaticBoxSizer(charsetBox, wxHORIZONTAL);
  mixHSizer->Add(charsetSizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 1);
...
// 2D + Compress
  ...
  storageSizer->Add(compressCtrl, 0, wxALIGN_LEFT | wxALL, 1);
  wxCheckBox *rtreeCtrl = new wxCheckBox(this, ID_LDSHP_RTREE,
                                         wxT("With Spatial Index (R*Tree)"),
                                         wxDefaultPosition, wxDefaultSize);
  rtreeCtrl->SetValue(SpatialIndex);
  storageSizer->Add(rtreeCtrl, 0, wxALIGN_LEFT | wxALL, 1);
// column-names Lower/Uppercase
  wxString case_selection[3];
  case_selection[0] = wxT("convert to Lowercase");
  case_selection[1] = wxT("convert to Uppercase");
  case_selection[2] = wxT("never change");
  wxRadioBox *case_sel = new wxRadioBox(this, ID_LDSHP_COLNAME_CASE,
                                        wxT("&Column Names"),
                                        wxDefaultPosition,
                                        wxDefaultSize, 3,
                                        case_selection, 3,
                                        wxRA_SPECIFY_ROWS);
  case_sel->SetSelection(0);
  compressBox->AddSpacer(40);
  compressBox->Add(case_sel, 0, wxALIGN_RIGHT | wxALL, 1);
...
// appends event handler for OK button
  ...
  Connect(ID_LDSHP_STATISTICS, wxEVT_COMMAND_RADIOBOX_SELECTED,
          (wxObjectEventFunction) & LoadShpDialog::OnUpdateStatistics);
}
...
void LoadShpDialog::OnUpdateStatistics(wxCommandEvent & WXUNUSED(event))
{
//
// the mode of user defined UpdateStatistics changed
//
  wxRadioBox *radioCtrl = (wxRadioBox *) FindWindow(ID_LDSHP_STATISTICS);
  if (radioCtrl->GetSelection() == 0)
...
//
  ...
  wxRadioBox *case_sel = (wxRadioBox *) FindWindow(ID_LDSHP_COLNAME_CASE);
  if (case_sel->GetSelection() == 0)
    ColnameCase = GAIA_DBF_COLNAME_LOWERCASE;
  else if (case_sel->GetSelection() == 1)
    ColnameCase = GAIA_DBF_COLNAME_UPPERCASE;
...
LoadZipShpDialog::~LoadZipShpDialog()
{
// destructor
  if (PKFields != NULL)
    delete[]PKFields;
...
bool LoadZipShpDialog::Create(MyFrame * parent, const char *zip_path,
                              const char *basename, wxString & table, int srid,
...
//
  MainFrame = parent;
  ZipPath = wxString::FromUTF8(zip_path);
  Basename = wxString::FromUTF8(basename);
  Table = table;
  SRID = srid;
  Column = column;
  Default = defCs;
  ...
  UpdateStatistics = true;
  LoadPKFields();
  if (wxDialog::Create(parent, wxID_ANY, wxT("Load Shapefile (from Zipfile)"))
      == false)
    return false;
// populates individual controls
  CreateControls();
...
void LoadZipShpDialog::CreateControls()
{
//
// creating individual control and setting initial values
//
...
// first row: the Shapefile path
  ...
  box2Sizer->Add(baseSizer, 0, wxALIGN_RIGHT | wxALL, 0);
  wxStaticText *baseLabel =
    new wxStaticText(this, wxID_STATIC, wxT("Shapefile:"));
  baseSizer->Add(baseLabel, 0, wxALIGN_CENTER_VERTICAL | wxALL, 1);
  wxTextCtrl *baseValue = new wxTextCtrl(this, wxID_STATIC,
                                         Basename, wxDefaultPosition,
                                         wxSize(350, 22), wxTE_READONLY);
  baseSizer->Add(baseValue, 0, wxALIGN_RIGHT | wxALL, 1);
// second row: TABLE name
  wxBoxSizer *tableSizer = new wxBoxSizer(wxHORIZONTAL);
  box2Sizer->Add(tableSizer, 0, wxALIGN_RIGHT | wxALL, 0);
  wxStaticText *tableLabel =
...
// appends event handler for OK button
  Connect(wxID_OK, wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction) & LoadZipShpDialog::OnOk);
  Connect(ID_LDSHP_USER_GTYPE, wxEVT_COMMAND_RADIOBOX_SELECTED,
          (wxObjectEventFunction) & LoadZipShpDialog::OnUserGType);
  Connect(ID_LDSHP_USER_PKEY, wxEVT_COMMAND_RADIOBOX_SELECTED,
          (wxObjectEventFunction) & LoadZipShpDialog::OnUserPKey);
  Connect(ID_LDSHP_TEXTDATES, wxEVT_COMMAND_RADIOBOX_SELECTED,
          (wxObjectEventFunction) & LoadZipShpDialog::OnTextDates);
  Connect(ID_LDSHP_STATISTICS, wxEVT_COMMAND_RADIOBOX_SELECTED,
          (wxObjectEventFunction) & LoadZipShpDialog::OnUpdateStatistics);
}

void LoadZipShpDialog::LoadPKFields()
{
// loading the Primary Key Fields list
  char *zip_path;
  char *basename;
...
void LoadZipShpDialog::OnUserGType(wxCommandEvent & WXUNUSED(event))
{
//
// the mode of user defined Geometry Type changed
//
...
void LoadZipShpDialog::OnUserPKey(wxCommandEvent & WXUNUSED(event))
{
//
// the mode of user defined Primary Key Column changed
//
...
void LoadZipShpDialog::OnTextDates(wxCommandEvent & WXUNUSED(event))
{
//
// the mode of user defined DBF TextDates changed
//
...
void LoadZipShpDialog::OnUpdateStatistics(wxCommandEvent & WXUNUSED(event))
{
//
// the mode of user defined UpdateStatistics changed
//
...
void LoadZipShpDialog::OnOk(wxCommandEvent & WXUNUSED(event))
{
//
// all done: 
//
...
bool CloneTableDialog::Create(MyFrame * parent, wxString & db_prefix,
                              wxString & in_table)
{
//
// creating the dialog
//
  ...
  dbPrefix = db_prefix;
  inTable = in_table;
  if (wxDialog::Create(parent, wxID_ANY, wxT("Clone Table")) == false)
    return false;
// populates individual controls
  CreateControls();
// sets dialog sizer
...
void CloneTableDialog::CreateControls()
{
//
// creating individual control and setting initial values
//
...
// first row: the INPUT TABLE selection
  wxBoxSizer *inSizer = new wxBoxSizer(wxHORIZONTAL);
  row0Sizer->Add(inSizer, 0, wxALIGN_CENTRE_VERTICAL | wxALL, 0);
  wxStaticBox *inBox = new wxStaticBox(this, wxID_STATIC,
                                       wxT("Origin (input table)"),
                                       wxDefaultPosition,
                                       wxDefaultSize);
  wxBoxSizer *intblSizer = new wxStaticBoxSizer(inBox, wxVERTICAL);
  inSizer->Add(intblSizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 5);
  char *xname = (char *) malloc((dbPrefix.Len() * 4) + 1);
  strcpy(xname, dbPrefix.ToUTF8());
  char *xname2 = gaiaDoubleQuotedSql(xname);
  free(xname);
  wxString origin = wxT("\"") + wxString::FromUTF8(xname2) + wxT("\".\"");
  free(xname2);
  ...
  free(xname2);
  wxTextCtrl *inpTable =
    new wxTextCtrl(this, wxID_ANY, origin, wxDefaultPosition,
                   wxSize(450, 22), wxTE_READONLY);
  intblSizer->Add(inpTable, 0, wxALIGN_CENTER_VERTICAL | wxALL, 0);
// second row: the OUTPUT TABLE selection
  wxBoxSizer *outSizer = new wxBoxSizer(wxHORIZONTAL);
  row0Sizer->Add(outSizer, 0, wxALIGN_CENTRE_VERTICAL | wxALL, 0);
  wxStaticBox *outBox = new wxStaticBox(this, wxID_STATIC,
                                        wxT("Destination (output table)"),
                                        wxDefaultPosition,
                                        wxDefaultSize);
  wxBoxSizer *outtblSizer = new wxStaticBoxSizer(outBox, wxVERTICAL);
  outSizer->Add(outtblSizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 5);
  wxTextCtrl *outTable =
    new wxTextCtrl(this, ID_CLONE_OUTPUT, wxT(""), wxDefaultPosition,
                   wxSize(450, 22));
  outtblSizer->Add(outTable, 0, wxALIGN_CENTER_VERTICAL | wxALL, 0);
// thirdd row: clone OPTIONS
  wxBoxSizer *optSizer = new wxBoxSizer(wxHORIZONTAL);
  row0Sizer->Add(optSizer, 0, wxALIGN_CENTRE_VERTICAL | wxALL, 0);
  wxStaticBox *optBox = new wxStaticBox(this, wxID_STATIC,
                                        wxT("Clone Options"),
                                        wxDefaultPosition,
                                        wxDefaultSize);
  wxBoxSizer *optionsSizer = new wxStaticBoxSizer(optBox, wxVERTICAL);
  optSizer->Add(optionsSizer, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
// Just Create
  wxCheckBox *JustCreate =
    new wxCheckBox(this, ID_CLONE_JUST_CREATE, wxT("&Just create (no copy)"));
  JustCreate->SetValue(false);
  optionsSizer->Add(JustCreate, 0, wxALIGN_LEFT | wxALL, 1);
// WithFK
  wxCheckBox *WithFK =
    new wxCheckBox(this, ID_CLONE_WITH_FK, wxT("&With Foreign Keys"));
  WithFK->SetValue(false);
  optionsSizer->Add(WithFK, 0, wxALIGN_LEFT | wxALL, 1);
// WithTriggers
  wxCheckBox *WithTriggers =
    new wxCheckBox(this, ID_CLONE_WITH_TRIGGERS, wxT("&With Triggers"));
  WithTriggers->SetValue(false);
  optionsSizer->Add(WithTriggers, 0, wxALIGN_LEFT | wxALL, 1);
// Resequence
  wxCheckBox *Resequence =
    new wxCheckBox(this, ID_CLONE_RESEQUENCE, wxT("&Resequence"));
  Resequence->SetValue(false);
  optionsSizer->Add(Resequence, 0, wxALIGN_LEFT | wxALL, 1);
// Append
  wxCheckBox *AppendMode =
    new wxCheckBox(this, ID_CLONE_APPEND, wxT("&Append mode"));
  AppendMode->SetValue(false);
  optionsSizer->Add(AppendMode, 0, wxALIGN_LEFT | wxALL, 1);
// columns to be ignored
  wxStaticBox *colsBox = new wxStaticBox(this, wxID_STATIC,
                                         wxT("Columns to be ignored"),
                                         wxDefaultPosition,
                                         wxDefaultSize);
  wxBoxSizer *colsNameSizer = new wxStaticBoxSizer(colsBox, wxVERTICAL);
  optSizer->Add(colsNameSizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  int nCols;
  wxString *columns = MainFrame->GetTableColumns(dbPrefix, inTable, &nCols);
  wxSize size = wxSize(160, 160);
  wxListBox *colsCtrl = new wxListBox();
  colsCtrl->Create(this, ID_CLONE_IGNORE, wxDefaultPosition, size, nCols,
                   columns, wxLB_MULTIPLE | wxLB_HSCROLL);
  colsCtrl->SetSelection(0);
  delete[]columns;
  colsNameSizer->Add(colsCtrl, 0, wxALIGN_CENTER_VERTICAL | wxALL, 0);
// geometries to be casted to MultiType
  wxStaticBox *geomsBox = new wxStaticBox(this, wxID_STATIC,
                                          wxT("Cast to MultiGeomType"),
                                          wxDefaultPosition,
                                          wxDefaultSize);
  wxBoxSizer *geomsNameSizer = new wxStaticBoxSizer(geomsBox, wxVERTICAL);
  optSizer->Add(geomsNameSizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  columns = MainFrame->GetTableGeometries(dbPrefix, inTable, &nCols);
  wxListBox *geomsCtrl = new wxListBox();
  geomsCtrl->Create(this, ID_CLONE_CAST2MULTI, wxDefaultPosition, size, nCols,
                    columns, wxLB_MULTIPLE | wxLB_HSCROLL);
  geomsCtrl->SetSelection(0);
  delete[]columns;
  geomsNameSizer->Add(geomsCtrl, 0, wxALIGN_CENTER_VERTICAL | wxALL, 0);
// OK - CANCEL buttons
  wxBoxSizer *okCancelBox = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(okCancelBox, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxButton *ok = new wxButton(this, wxID_OK, wxT("&OK"));
...
// setting up an event handler [dialog closing]
  Connect(wxID_OK, wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction) & CloneTableDialog::OnOk);
// appends event handlers for radio buttons etc
}

void CloneTableDialog::GetSQL(wxString & sql)
{
//
// preparing the SQL statement
//
  wxTextCtrl *outTable = (wxTextCtrl *) FindWindow(ID_CLONE_OUTPUT);
  wxCheckBox *JustCreate = (wxCheckBox *) FindWindow(ID_CLONE_JUST_CREATE);
  wxCheckBox *WithFK = (wxCheckBox *) FindWindow(ID_CLONE_WITH_FK);
  wxCheckBox *WithTriggers = (wxCheckBox *) FindWindow(ID_CLONE_WITH_TRIGGERS);
  wxCheckBox *Resequence = (wxCheckBox *) FindWindow(ID_CLONE_RESEQUENCE);
  wxCheckBox *Append = (wxCheckBox *) FindWindow(ID_CLONE_APPEND);
  wxListBox *ignoreCtrl = (wxListBox *) FindWindow(ID_CLONE_IGNORE);
  wxListBox *castCtrl = (wxListBox *) FindWindow(ID_CLONE_CAST2MULTI);

  if (JustCreate->GetValue() == true)
    sql = wxT("SELECT CreateClonedTable('");
  else
    sql = wxT("SELECT CloneTable('");
  char *xname = (char *) malloc((dbPrefix.Len() * 4) + 1);
  strcpy(xname, dbPrefix.ToUTF8());
  char *xname2 = gaiaSingleQuotedSql(xname);
  free(xname);
  ...
  free(xname2);
  sql += wxT("', '");
  wxString out = outTable->GetValue();
  xname = (char *) malloc((out.Len() * 4) + 1);
  strcpy(xname, out.ToUTF8());
  xname2 = gaiaSingleQuotedSql(xname);
  free(xname);
  sql += wxString::FromUTF8(xname2) + wxT("', 1");
  free(xname2);
  if (WithFK->GetValue() == true)
    sql += wxT(", '::with-foreign-keys::'");
  if (WithTriggers->GetValue() == true)
    sql += wxT(", '::with-triggers::'");
  if (Resequence->GetValue() == true)
    sql += wxT(", '::resequence::'");
  if (Append->GetValue() == true)
    sql += wxT(", '::append::'");
  for (unsigned int i = 1; i < ignoreCtrl->GetCount(); i++)
    {
      if (ignoreCtrl->IsSelected(i) == true)
        {
          wxString str = ignoreCtrl->GetString(i);
          xname = (char *) malloc((str.Len() * 4) + 1);
          strcpy(xname, str.ToUTF8());
          xname2 = gaiaSingleQuotedSql(xname);
          free(xname);
          sql += wxT(", '::ignore::") + wxString::FromUTF8(xname2) + wxT("'");
          free(xname2);
        }
    }
  for (unsigned int i = 1; i < castCtrl->GetCount(); i++)
    {
      if (castCtrl->IsSelected(i) == true)
        {
          wxString str = castCtrl->GetString(i);
          xname = (char *) malloc((str.Len() * 4) + 1);
          strcpy(xname, str.ToUTF8());
          xname2 = gaiaSingleQuotedSql(xname);
          free(xname);
          sql +=
            wxT(", ':cast2multi::") + wxString::FromUTF8(xname2) + wxT("'");
          free(xname2);
        }
    }
...
void CloneTableDialog::OnOk(wxCommandEvent & WXUNUSED(event))
{
//
// all done: 
//
  ...
  if (out.Len() < 1)
    {
      wxMessageBox(wxT("You must specify some NAME for the OUTPUT TABLE !!!"),
                   wxT("spatialite_gui"), wxOK | wxICON_WARNING, this);
      return;
    }
  bool exists = MainFrame->TableAlreadyExists(out);
  if (exists && JustCreate->GetValue() == true)
    {
      wxMessageBox(wxT("The OUTPUT TABLE already exists: cannot create twice"),
                   wxT("spatialite_gui"), wxOK | wxICON_WARNING, this);
      return;
    }
  if (exists && Append->GetValue() == false)
    {
      wxMessageBox(wxT("The OUTPUT TABLE already exists:\n"
                       "please consider selecting the APPEND option"),
                   wxT("spatialite_gui"), wxOK | wxICON_WARNING, this);
      return;
    }
  wxDialog::EndModal(wxID_OK);
...
//
  ...
  VirtualTable = wxT("");
  Overwrite = false;
  if (wxDialog::Create(parent, wxID_ANY, wxT("Create Network")) == false)
    return false;
// populates individual controls
  CreateControls();
// sets dialog sizer
...
// output tables
  ...
  wxStaticBox *outTablesBox = new wxStaticBox(this, wxID_STATIC,
                                              ...
                                              wxDefaultPosition,
                                              wxDefaultSize);
  wxBoxSizer *outTablesSizer = new wxStaticBoxSizer(outTablesBox, wxVERTICAL);
  outSizer->Add(outTablesSizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxBoxSizer *out1Sizer = new wxBoxSizer(wxHORIZONTAL);
  outTablesSizer->Add(out1Sizer, 0, wxALIGN_CENTRE_HORIZONTAL | wxALL, 0);
  wxStaticText *dataTableLabel =
    new wxStaticText(this, wxID_STATIC, wxT("&binary data Table:"));
  out1Sizer->Add(dataTableLabel, 0, wxALIGN_CENTER_VERTICAL | wxALL, 1);
  wxTextCtrl *dataTableCtrl = new wxTextCtrl(this, ID_NET_DATA, wxT(""),
                                             wxDefaultPosition, wxSize(150,
                                                                       22));
  out1Sizer->Add(dataTableCtrl, 0, wxALIGN_RIGHT | wxALL, 5);
  out1Sizer->AddSpacer(15);
  wxStaticText *virtualTableLabel =
    new wxStaticText(this, wxID_STATIC, wxT("&VirtualNetwork Table:"));
  out1Sizer->Add(virtualTableLabel, 0, wxALIGN_CENTER_VERTICAL | wxALL, 1);
  wxTextCtrl *virtualTableCtrl = new wxTextCtrl(this, ID_NET_VIRTUAL, wxT(""),
                                                wxDefaultPosition, wxSize(150,
                                                                          22));
  out1Sizer->Add(virtualTableCtrl, 0, wxALIGN_RIGHT | wxALL, 5);
// Overwrite existing output tables
  wxBoxSizer *out2Sizer = new wxBoxSizer(wxHORIZONTAL);
  outTablesSizer->Add(out2Sizer, 0, wxALIGN_CENTRE_HORIZONTAL | wxALL, 0);
  wxCheckBox *overwrite = new wxCheckBox(this, ID_NET_OVERWRITE,
                                         wxT
                                         ("&Overwrite existing output tables"));
  out2Sizer->Add(overwrite, 0, wxALIGN_CENTRE_HORIZONTAL | wxALL, 1);
// OK - CANCEL buttons
  wxBoxSizer *okCancelBox = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(okCancelBox, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxButton *ok = new wxButton(this, wxID_OK, wxT("&OK"));
...
// appends event handlers for radio buttons etc
  ...
  Connect(ID_NET_OVERWRITE, wxEVT_COMMAND_CHECKBOX_CLICKED,
          (wxObjectEventFunction) & NetworkDialog::OnOverwrite);
}
...
void NetworkDialog::SetCurrentTable()
{
//
// The currently selected Table changed
//
...
void NetworkDialog::OnOverwrite(wxCommandEvent & WXUNUSED(event))
{
//
// Overwrite check box
//
...
bool GpsPicsDialog::Create(MyFrame * parent, wxString & dir_path,
                           wxString & img_path)
{
//
// creating the dialog
//
  ...
  UpdateStatistics = true;
  SpatialIndex = true;
  if (wxDialog::Create(parent, wxID_ANY, wxT("Import EXIF-GPS Photos")) ==
      false)
    return false;
// populates individual controls
  CreateControls();
...
void GpsPicsDialog::CreateControls()
{
//
// creating individual control and setting initial values
//
...
// first row: the path and file/folder selection
  ...
  wxBoxSizer *pthSizer = new wxStaticBoxSizer(pathBox, wxVERTICAL);
  pathSizer->Add(pthSizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 5);
  wxStaticText *pathLabel = new wxStaticText(this, ID_GPS_PICS_PATH, ImgPath);
  pthSizer->Add(pathLabel, 0, wxALIGN_CENTER_VERTICAL | wxALL, 0);
  wxString imgFolder[2];
  imgFolder[0] = wxT("Import &selected image only");
  imgFolder[1] = wxT("Import &any EXIF-GPS from selected folder");
  wxRadioBox *imgFolderSel = new wxRadioBox(this, ID_GPS_PICS_FOLDER,
                                            wxT("&Source selection"),
                                            wxDefaultPosition,
                                            wxDefaultSize, 2,
                                            imgFolder, 2,
                                            wxRA_SPECIFY_ROWS);
  imgFolderSel->SetSelection(1);
  pthSizer->Add(imgFolderSel, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 5);
// target Table / Geometry Column
  wxStaticBox *targetBox = new wxStaticBox(this, wxID_STATIC,
                                           wxT
                                           ("Destination: Table and GeometryColumn"),
                                           wxDefaultPosition, wxDefaultSize);
  ...
  wxTextCtrl *tableValue = new wxTextCtrl(this, ID_GPS_PICS_TABLE,
                                          wxT(""), wxDefaultPosition,
                                          wxSize(250, 22));
  tblSizer->Add(tableValue, 0, wxALIGN_RIGHT | wxALL, 0);
  wxBoxSizer *geomSizer = new wxBoxSizer(wxHORIZONTAL);
  targetSizer->Add(geomSizer, 0, wxALIGN_RIGHT | wxALL, 0);
  wxStaticText *geomLabel =
    new wxStaticText(this, wxID_STATIC, wxT("&Column name:"));
  geomSizer->Add(geomLabel, 0, wxALIGN_CENTER_VERTICAL | wxALL, 0);
  wxTextCtrl *geometryValue = new wxTextCtrl(this, ID_GPS_PICS_GEOMETRY,
                                             wxT("geometry"), wxDefaultPosition,
                                             wxSize(250, 22));
  geomSizer->Add(geometryValue, 0, wxALIGN_RIGHT | wxALL, 0);
// R*Tree + UpdateStatistics
  wxBoxSizer *mixBox = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(mixBox, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxStaticBox *optionsBox = new wxStaticBox(this, wxID_STATIC,
                                            wxT("Options"),
                                            wxDefaultPosition,
                                            wxDefaultSize);
  wxBoxSizer *optionsSizer = new wxStaticBoxSizer(optionsBox, wxVERTICAL);
  mixBox->Add(optionsSizer, 0, wxALIGN_LEFT | wxALL, 1);
  wxCheckBox *rtreeCtrl = new wxCheckBox(this, ID_GPS_PICS_RTREE,
                                         wxT("With Spatial Index (R*Tree)"),
                                         wxDefaultPosition, wxDefaultSize);
  rtreeCtrl->SetValue(SpatialIndex);
  optionsSizer->Add(rtreeCtrl, 0, wxALIGN_LEFT | wxALL, 1);
  wxCheckBox *statsCtrl = new wxCheckBox(this, ID_GPS_PICS_STATISTICS,
                                         wxT("Update Layer Statistics"),
                                         wxDefaultPosition, wxDefaultSize);
  statsCtrl->SetValue(UpdateStatistics);
  optionsSizer->Add(statsCtrl, 0, wxALIGN_LEFT | wxALL, 1);
// OK - CANCEL buttons
  wxBoxSizer *okCancelBox = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(okCancelBox, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxButton *ok = new wxButton(this, wxID_OK, wxT("&OK"));
...
// appends event handler for OK button
  Connect(wxID_OK, wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction) & GpsPicsDialog::OnOk);
// appends event handlers for radio buttons etc
  Connect(ID_GPS_PICS_FOLDER, wxEVT_COMMAND_RADIOBOX_SELECTED,
          (wxObjectEventFunction) & GpsPicsDialog::OnFolder);
  Connect(ID_GPS_PICS_RTREE, wxEVT_COMMAND_RADIOBOX_SELECTED,
          (wxObjectEventFunction) & GpsPicsDialog::OnSpatialIndex);
  Connect(ID_GPS_PICS_STATISTICS, wxEVT_COMMAND_RADIOBOX_SELECTED,
          (wxObjectEventFunction) & GpsPicsDialog::OnUpdateStatistics);
}

void GpsPicsDialog::OnFolder(wxCommandEvent & WXUNUSED(event))
{
//
// File/Folder radio box
//
  wxRadioBox *folderSel = (wxRadioBox *) FindWindow(ID_GPS_PICS_FOLDER);
  wxStaticText *pathLabel = (wxStaticText *) FindWindow(ID_GPS_PICS_PATH);
  if (Folder == true)
    {
      Folder = false;
      folderSel->SetSelection(0);
...
void GpsPicsDialog::OnSpatialIndex(wxCommandEvent & WXUNUSED(event))
{
//
// the mode of user defined SpatialIndex changed
//
  wxRadioBox *radioCtrl = (wxRadioBox *) FindWindow(ID_GPS_PICS_RTREE);
  if (radioCtrl->GetSelection() == 0)
    SpatialIndex = false;
  else
    SpatialIndex = true;
...
void GpsPicsDialog::OnUpdateStatistics(wxCommandEvent & WXUNUSED(event))
{
//
// the mode of user defined UpdateStatistics changed
//
  wxRadioBox *radioCtrl = (wxRadioBox *) FindWindow(ID_GPS_PICS_STATISTICS);
  if (radioCtrl->GetSelection() == 0)
    UpdateStatistics = false;
  else
    UpdateStatistics = true;
...
void GpsPicsDialog::OnOk(wxCommandEvent & WXUNUSED(event))
{
//
// all done: 
//
  wxTextCtrl *tableCtrl = (wxTextCtrl *) FindWindow(ID_GPS_PICS_TABLE);
  Table = tableCtrl->GetValue();
  if (Table.Len() < 1)
    {
  ...
  wxTextCtrl *geomCtrl = (wxTextCtrl *) FindWindow(ID_GPS_PICS_GEOMETRY);
  Geometry = geomCtrl->GetValue();
  if (Geometry.Len() < 1)
    {
      wxMessageBox(wxT("You must specify some Geometry column name !!!"),
                   wxT("spatialite_gui"), wxOK | wxICON_WARNING, this);
      return;
    }
  wxDialog::EndModal(wxID_OK);
...
bool AddAllRastersSridDialog::Create(MyFrame * parent)
{
//
// creating the dialog
//
...
void AddAllRastersSridDialog::CreateControls()
{
//
// creating individual control and setting initial values
//
  ...
  wxBoxSizer *boxSizer = new wxBoxSizer(wxVERTICAL);
  topSizer->Add(boxSizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 5);
// first row: SRID to be added
  wxBoxSizer *newSizer = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(newSizer, 0, wxALIGN_LEFT | wxALL, 0);
  wxStaticBox *newBox = new wxStaticBox(this, wxID_STATIC,
                                        wxT("SRID to be added"),
                                        wxDefaultPosition,
                                        wxDefaultSize);
  wxBoxSizer *new1Sizer = new wxStaticBoxSizer(newBox, wxVERTICAL);
  newSizer->Add(new1Sizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 5);
  wxBoxSizer *new2Sizer = new wxBoxSizer(wxHORIZONTAL);
  new1Sizer->Add(new2Sizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxSpinCtrl *sridNewCtrl =
    new wxSpinCtrl(this, ID_ADDALL_SRID_NEW, wxEmptyString,
                   wxDefaultPosition, wxSize(80, 20),
                   wxSP_ARROW_KEYS,
                   -1, 1000000, -1);
  new2Sizer->Add(sridNewCtrl, 0, wxALIGN_LEFT | wxALL, 5);
  new2Sizer->AddSpacer(25);
  wxTextCtrl *nameNewCtrl =
    new wxTextCtrl(this, ID_ADDALL_NAME_NEW, wxT("Undefined"),
                   wxDefaultPosition, wxSize(350, 22),
                   wxTE_READONLY);
  new2Sizer->Add(nameNewCtrl, 0, wxALIGN_RIGHT | wxALL, 5);
// second row: already defined SRID 
  wxBoxSizer *oldSizer = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(oldSizer, 0, wxALIGN_LEFT | wxALL, 0);
  wxStaticBox *oldBox = new wxStaticBox(this, wxID_STATIC,
                                        wxT("reference SRID"),
                                        wxDefaultPosition,
                                        wxDefaultSize);
  wxBoxSizer *old1Sizer = new wxStaticBoxSizer(oldBox, wxVERTICAL);
  oldSizer->Add(old1Sizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 5);
  wxBoxSizer *old2Sizer = new wxBoxSizer(wxHORIZONTAL);
  old1Sizer->Add(old2Sizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxSpinCtrl *sridoldCtrl =
    new wxSpinCtrl(this, ID_ADDALL_SRID_OLD, wxEmptyString,
                   wxDefaultPosition, wxSize(80, 20),
                   wxSP_ARROW_KEYS,
                   -1, 1000000, -1);
  old2Sizer->Add(sridoldCtrl, 0, wxALIGN_LEFT | wxALL, 5);
  old2Sizer->AddSpacer(25);
  wxTextCtrl *nameoldCtrl =
    new wxTextCtrl(this, ID_ADDALL_NAME_OLD, wxT("Undefined"),
                   wxDefaultPosition, wxSize(350, 22),
                   wxTE_READONLY);
  old2Sizer->Add(nameoldCtrl, 0, wxALIGN_RIGHT | wxALL, 5);
  wxBoxSizer *old3Sizer = new wxBoxSizer(wxVERTICAL);
  old1Sizer->Add(old3Sizer, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxString note =
    wxT("INFO: the new SRID will be added (if not already defined)");
  wxStaticText *note1Label = new wxStaticText(this, wxID_STATIC, note);
  old3Sizer->Add(note1Label, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
  note = wxT("to all Raster Coverages already declaring this SRID");
  wxStaticText *note2Label = new wxStaticText(this, wxID_STATIC, note);
  old3Sizer->Add(note2Label, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
// ADD / QUIT button
  wxBoxSizer *okCancelBox = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(okCancelBox, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxButton *ok = new wxButton(this, wxID_OK, wxT("&Add"));
  ok->Enable(false);
  okCancelBox->Add(ok, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
...
// appends event handler for OK button
  Connect(wxID_OK, wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction) & AddAllRastersSridDialog::OnOk);
  Connect(ID_ADDALL_SRID_NEW, wxEVT_COMMAND_SPINCTRL_UPDATED,
          (wxObjectEventFunction) & AddAllRastersSridDialog::OnNewSridChanged);
  Connect(ID_ADDALL_SRID_NEW, wxEVT_COMMAND_TEXT_UPDATED,
          (wxObjectEventFunction) & AddAllRastersSridDialog::OnNewSridChanged);
  Connect(ID_ADDALL_SRID_OLD, wxEVT_COMMAND_SPINCTRL_UPDATED,
          (wxObjectEventFunction) & AddAllRastersSridDialog::OnOldSridChanged);
  Connect(ID_ADDALL_SRID_OLD, wxEVT_COMMAND_TEXT_UPDATED,
          (wxObjectEventFunction) & AddAllRastersSridDialog::OnOldSridChanged);
}

bool AddAllRastersSridDialog::FetchRefSysName(int srid, wxString & name)
{
//
...
void AddAllRastersSridDialog::OnNewSridChanged(wxCommandEvent & WXUNUSED(event))
{
//
// NEW Srid changed
//
  wxSpinCtrl *sridNewCtrl = (wxSpinCtrl *) FindWindow(ID_ADDALL_SRID_NEW);
  wxTextCtrl *nameNewCtrl = (wxTextCtrl *) FindWindow(ID_ADDALL_NAME_NEW);
  wxSpinCtrl *sridOldCtrl = (wxSpinCtrl *) FindWindow(ID_ADDALL_SRID_OLD);
  wxButton *ok = (wxButton *) FindWindow(wxID_OK);
  int newSrid = sridNewCtrl->GetValue();
  int oldSrid = sridOldCtrl->GetValue();
  wxString name;
  bool valid_new = FetchRefSysName(newSrid, name);
  nameNewCtrl->SetValue(name);
  bool valid_old = FetchRefSysName(oldSrid, name);
...
void AddAllRastersSridDialog::OnOldSridChanged(wxCommandEvent & WXUNUSED(event))
{
//
// OLD Srid changed
//
  wxSpinCtrl *sridNewCtrl = (wxSpinCtrl *) FindWindow(ID_ADDALL_SRID_NEW);
  wxTextCtrl *nameOldCtrl = (wxTextCtrl *) FindWindow(ID_ADDALL_NAME_OLD);
  wxSpinCtrl *sridOldCtrl = (wxSpinCtrl *) FindWindow(ID_ADDALL_SRID_OLD);
  wxButton *ok = (wxButton *) FindWindow(wxID_OK);
  int newSrid = sridNewCtrl->GetValue();
  int oldSrid = sridOldCtrl->GetValue();
...
void AddAllRastersSridDialog::OnOk(wxCommandEvent & WXUNUSED(event))
{
//
// inserting the new alternative SRIDs
//
  sqlite3 *sqlite = MainFrame->GetSqlite();
  int i;
  char **results;
  ...
  char *sql =
    sqlite3_mprintf
    ("SELECT SE_RegisterRasterCoverageSrid( z.coverage_name, %d) FROM "
     "(SELECT c.coverage_name AS coverage_name FROM MAIN.raster_coverages AS c "
     "WHERE c.srid = %d UNION "
     "SELECT coverage_name AS coverage_name FROM MAIN.raster_coverages_srid "
     "WHERE srid = %d) AS z", newSrid, oldSrid, oldSrid);
...
stop:
  sqlite3_free(sql);
  char *msg =
    sqlite3_mprintf("Inserted %d alternative SRID definition%c", inserted,
                    (inserted > 1) ? 's' : '\0');
...
bool AddAllVectorsSridDialog::Create(MyFrame * parent)
{
//
// creating the dialog
//
...
void AddAllVectorsSridDialog::CreateControls()
{
//
// creating individual control and setting initial values
//
...
// second row: already defined SRID 
  ...
  wxStaticText *note1Label = new wxStaticText(this, wxID_STATIC, note);
  old3Sizer->Add(note1Label, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
  note = wxT("to all Vector Coverages already declaring this SRID");
  wxStaticText *note2Label = new wxStaticText(this, wxID_STATIC, note);
  old3Sizer->Add(note2Label, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
// ADD / QUIT button
  wxBoxSizer *okCancelBox = new wxBoxSizer(wxHORIZONTAL);
...
// appends event handler for OK button
  Connect(wxID_OK, wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction) & AddAllVectorsSridDialog::OnOk);
  Connect(ID_ADDALL_SRID_NEW, wxEVT_COMMAND_SPINCTRL_UPDATED,
          (wxObjectEventFunction) & AddAllVectorsSridDialog::OnNewSridChanged);
  Connect(ID_ADDALL_SRID_NEW, wxEVT_COMMAND_TEXT_UPDATED,
          (wxObjectEventFunction) & AddAllVectorsSridDialog::OnNewSridChanged);
  Connect(ID_ADDALL_SRID_OLD, wxEVT_COMMAND_SPINCTRL_UPDATED,
          (wxObjectEventFunction) & AddAllVectorsSridDialog::OnOldSridChanged);
  Connect(ID_ADDALL_SRID_OLD, wxEVT_COMMAND_TEXT_UPDATED,
          (wxObjectEventFunction) & AddAllVectorsSridDialog::OnOldSridChanged);
}

bool AddAllVectorsSridDialog::FetchRefSysName(int srid, wxString & name)
{
//
// fetching the RefSys Name
//
...
void AddAllVectorsSridDialog::OnNewSridChanged(wxCommandEvent & WXUNUSED(event))
{
//
// NEW Srid changed
//
...
void AddAllVectorsSridDialog::OnOldSridChanged(wxCommandEvent & WXUNUSED(event))
{
//
// OLD Srid changed
//
...
void AddAllVectorsSridDialog::OnOk(wxCommandEvent & WXUNUSED(event))
{
//
// inserting the new alternative SRIDs
//
  ...
  char *sql =
    sqlite3_mprintf
    ("SELECT SE_RegisterVectorCoverageSrid( z.coverage_name, %d) FROM "
     "(SELECT c.coverage_name AS coverage_name FROM MAIN.vector_coverages AS c "
     "JOIN MAIN.geometry_columns AS s ON (c.topology_name IS NULL AND c.network_name IS NULL AND "
     "c.f_table_name IS NOT NULL AND c.f_geometry_column IS NOT NULL AND "
     "c.f_table_name = s.f_table_name AND c.f_geometry_column = s.f_geometry_column) "
     "WHERE s.srid = %d UNION "
     "SELECT c.coverage_name AS coverage_name FROM MAIN.vector_coverages AS c "
     "JOIN MAIN.views_geometry_columns AS x ON (c.view_name IS NOT NULL AND c.view_geometry IS NOT NULL "
     "AND c.view_name = x.view_name AND c.view_geometry = x.view_geometry) "
     "JOIN MAIN.geometry_columns AS s ON (x.f_table_name = s.f_table_name AND "
     "x.f_geometry_column = s.f_geometry_column) WHERE s.srid = %d UNION "
     "SELECT c.coverage_name AS coverage_name FROM MAIN.vector_coverages AS c "
     "JOIN MAIN.virts_geometry_columns AS s ON (c.virt_name IS NOT NULL AND c.virt_geometry IS NOT NULL "
     "AND c.virt_name = s.virt_name AND c.virt_geometry = s.virt_geometry) "
     "WHERE s.srid = %d UNION "
     "SELECT c.coverage_name AS coverage_name FROM MAIN.vector_coverages AS c "
     "JOIN MAIN.topologies AS s ON (c.topology_name IS NOT NULL AND c.topology_name = s.topology_name) "
     "WHERE s.srid = %d UNION "
     "SELECT c.coverage_name AS coverage_name FROM MAIN.vector_coverages AS c "
     "JOIN MAIN.networks AS s ON (c.network_name IS NOT NULL AND c.network_name = s.network_name) "
     "WHERE s.srid = %d UNION "
     "SELECT coverage_name AS coverage_name FROM MAIN.vector_coverages_srid "
     "WHERE srid = %d) AS z", newSrid, oldSrid, oldSrid, oldSrid, oldSrid,
     oldSrid, oldSrid);
...
bool LoadGeoJsonDialog::Create(MyFrame * parent, wxString & path,
...
//
  ...
  SpatialIndex = false;
  UpdateStatistics = true;
  if (wxDialog::Create(parent, wxID_ANY, wxT("Load GeoJSON")) == false)
    return false;
// populates individual controls
  CreateControls();
// sets dialog sizer
...
void LoadGeoJsonDialog::CreateControls()
{
//
// creating individual control and setting initial values
//
...
// fourth row: SRID
  ...
  mixHSizer->Add(mixVSizer, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
  wxBoxSizer *sridSizer = new wxBoxSizer(wxHORIZONTAL);
  mixVSizer->Add(sridSizer, 0, wxALIGN_LEFT | wxALL, 0);
  wxStaticText *sridLabel = new wxStaticText(this, wxID_STATIC, wxT("&SRID:"));
  sridSizer->Add(sridLabel, 0, wxALIGN_CENTER_VERTICAL | wxALL, 1);
  wxSpinCtrl *sridCtrl = new wxSpinCtrl(this, ID_LDSHP_SRID, wxEmptyString,
                                        wxDefaultPosition, wxSize(80, 20),
...
// UpdateStatistics
  wxBoxSizer *statisticsBox = new wxBoxSizer(wxHORIZONTAL);
  mixVSizer->Add(statisticsBox, 0, wxALIGN_LEFT | wxALL, 0);
  wxStaticBox *statBox = new wxStaticBox(this, wxID_STATIC,
                                         wxT("Layer Statistics"),
                                         wxDefaultPosition,
                                         wxDefaultSize);
  ...
  wxCheckBox *rtreeCtrl = new wxCheckBox(this, ID_LDSHP_RTREE,
                                         ...
                                         wxDefaultPosition, wxDefaultSize);
  rtreeCtrl->SetValue(SpatialIndex);
  mixVSizer->Add(rtreeCtrl, 0, wxALIGN_LEFT | wxALL, 1);
// column-names Lower/Uppercase
  wxString case_selection[3];
  case_selection[0] = wxT("convert to Lowercase");
  case_selection[1] = wxT("convert to Uppercase");
  ...
  wxRadioBox *case_sel = new wxRadioBox(this, ID_LDSHP_COLNAME_CASE,
                                        ...
                                        wxRA_SPECIFY_ROWS);
  case_sel->SetSelection(0);
  mixHSizer->Add(case_sel, 0, wxALIGN_CENTER | wxALL, 1);
// OK - CANCEL buttons
  wxBoxSizer *okCancelBox = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(okCancelBox, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxButton *ok = new wxButton(this, wxID_OK, wxT("&OK"));
...
// appends event handler for OK button
  Connect(wxID_OK, wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction) & LoadGeoJsonDialog::OnOk);
  Connect(ID_LDSHP_STATISTICS, wxEVT_COMMAND_RADIOBOX_SELECTED,
          (wxObjectEventFunction) & LoadGeoJsonDialog::OnUpdateStatistics);
}

void LoadGeoJsonDialog::OnUpdateStatistics(wxCommandEvent & WXUNUSED(event))
{
//
// the mode of user defined UpdateStatistics changed
//
...
void LoadGeoJsonDialog::OnOk(wxCommandEvent & WXUNUSED(event))
{
//
// all done: 
//
...
bool VirtualGeoJsonDialog::Create(MyFrame * parent, wxString & path,
                                  wxString & table)
{
//
// creating the dialog
//
  ...
  Table = table;
  SRID = 4326;
  if (wxDialog::Create(parent, wxID_ANY, wxT("Virtual GeoJSON")) == false)
    return false;
// populates individual controls
  CreateControls();
// sets dialog sizer
...
void VirtualGeoJsonDialog::CreateControls()
{
//
// creating individual control and setting initial values
//
...
// appends event handler for OK button
  Connect(wxID_OK, wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction) & VirtualGeoJsonDialog::OnOk);
}

void VirtualGeoJsonDialog::OnOk(wxCommandEvent & WXUNUSED(event))
{
//
// all done: 
//
...
ChoooseZipShpDialog::~ChoooseZipShpDialog()
{
// dtor
  int i;
  for (i = 0; i < Count; i++)
    {
      char *basename = *(Shapefiles + i);
      if (basename != NULL)
        free(basename);
    }
  free(Shapefiles);
...
bool ChoooseZipShpDialog::Create(MyFrame * parent, const char *zip_path,
                                 int count)
{
//
// creating the dialog
//
  ...
  Count = count;
  Basename = NULL;
  Shapefiles = (char **) malloc(sizeof(char *) * Count);
  for (i = 0; i < Count; i++)
    {
      char *basename = gaiaZipfileShpN(zip_path, i + 1);
      *(Shapefiles + i) = basename;
    }
  if (wxDialog::Create(parent, wxID_ANY, wxT("Shapefiles in Zipfile")) == false)
    return false;
// populates individual controls
  CreateControls();
// sets dialog sizer
...
void ChoooseZipShpDialog::CreateControls()
{
//
// creating individual control and setting initial values
//
  ...
  wxBoxSizer *msgSizer = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(msgSizer, 0, wxALIGN_RIGHT | wxALL, 0);
// First row: list of Shapefiles within the Zipfile
  wxBoxSizer *shpSizer = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(shpSizer, 0, wxALIGN_RIGHT | wxALL, 0);
  wxStaticText *shpLabel =
    new wxStaticText(this, wxID_STATIC, wxT("&List of Shapefiles:"));
  shpSizer->Add(shpLabel, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
  wxComboBox *shpList =
    new wxComboBox(this, ID_ZIPSHP, wxT(""), wxDefaultPosition,
                   wxSize(300, 21), 0, NULL, wxCB_DROPDOWN | wxCB_READONLY);
  for (i = 0; i < Count; i++)
    {
      char *basename = *(Shapefiles + i);
      if (basename != NULL)
        {
          wxString str = wxString::FromUTF8(basename);
          shpList->Append(str);
      } else
        shpList->Append(wxT("??? invalid ???"));
    }
  shpList->SetSelection(0);
  shpSizer->Add(shpList, 0, wxALIGN_RIGHT | wxALL, 5);
// OK - CANCEL buttons
  wxBoxSizer *okCancelBox = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(okCancelBox, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxButton *ok = new wxButton(this, wxID_OK, wxT("&OK"));
...
// appends event handler for OK button
  Connect(wxID_OK, wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction) & ChoooseZipShpDialog::OnOk);
}

void ChoooseZipShpDialog::OnOk(wxCommandEvent & WXUNUSED(event))
{
  int idSel;
  wxComboBox *shpList = (wxComboBox *) FindWindow(ID_ZIPSHP);
  idSel = shpList->GetSelection();
  if (idSel == wxNOT_FOUND)
    {
      wxMessageBox(wxT
                   ("you must select some Shapefile from the list"),
                   wxT("spatialite_gui"), wxOK | wxICON_WARNING, this);
      return;
    }
  Basename = *(Shapefiles + idSel);
  wxDialog::EndModal(wxID_OK);
}


ChoooseZipDbfDialog::~ChoooseZipDbfDialog()
{
// dtor
  int i;
  for (i = 0; i < Count; i++)
    {
      char *filename = *(DBFs + i);
      if (filename != NULL)
        free(filename);
    }
  free(DBFs);
...
bool ChoooseZipDbfDialog::Create(MyFrame * parent, const char *zip_path,
                                 int count)
{
//
// creating the dialog
//
  ...
  Count = count;
  Filename = NULL;
  DBFs = (char **) malloc(sizeof(char *) * Count);
  for (i = 0; i < Count; i++)
    {
      char *filename = gaiaZipfileDbfN(zip_path, i + 1);
      *(DBFs + i) = filename;
    }
  if (wxDialog::Create(parent, wxID_ANY, wxT("DBF files in Zipfile")) == false)
    return false;
// populates individual controls
  CreateControls();
// sets dialog sizer
...
void ChoooseZipDbfDialog::CreateControls()
{
//
// creating individual control and setting initial values
//
  ...
  boxSizer->Add(msgSizer, 0, wxALIGN_RIGHT | wxALL, 0);
// First row: list of Shapefiles within the Zipfile
  wxBoxSizer *dbfSizer = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(dbfSizer, 0, wxALIGN_RIGHT | wxALL, 0);
  wxStaticText *dbfLabel =
    new wxStaticText(this, wxID_STATIC, wxT("&List of DBF files:"));
  dbfSizer->Add(dbfLabel, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
  wxComboBox *dbfList =
    new wxComboBox(this, ID_ZIPDBF, wxT(""), wxDefaultPosition,
                   wxSize(300, 21), 0, NULL, wxCB_DROPDOWN | wxCB_READONLY);
  for (i = 0; i < Count; i++)
    {
      char *filename = *(DBFs + i);
      if (filename != NULL)
        {
          wxString str = wxString::FromUTF8(filename);
          dbfList->Append(str);
      } else
        dbfList->Append(wxT("??? invalid ???"));
    }
  dbfList->SetSelection(0);
  dbfSizer->Add(dbfList, 0, wxALIGN_RIGHT | wxALL, 5);
// OK - CANCEL buttons
  wxBoxSizer *okCancelBox = new wxBoxSizer(wxHORIZONTAL);
  boxSizer->Add(okCancelBox, 0, wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
  wxButton *ok = new wxButton(this, wxID_OK, wxT("&OK"));
...
// appends event handler for OK button
  Connect(wxID_OK, wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction) & ChoooseZipDbfDialog::OnOk);
}

void ChoooseZipDbfDialog::OnOk(wxCommandEvent & WXUNUSED(event))
{
  int idSel;
  wxComboBox *dbfList = (wxComboBox *) FindWindow(ID_ZIPDBF);
  idSel = dbfList->GetSelection();
  if (idSel == wxNOT_FOUND)
    {
      wxMessageBox(wxT
                   ("you must select some DBF file from the list"),
                   wxT("spatialite_gui"), wxOK | wxICON_WARNING, this);
      return;
    }
  Filename = *(DBFs + idSel);
