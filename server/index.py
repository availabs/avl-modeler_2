...
def viewByProjectId(projectId):
    ...
    persons= conn.execute(
        psql ).fetchall()  
    print("synpop households and persons", households[0].values(), persons[0].values())                  
...
def viewVarsByProjectId(projectId):
    ...
    hhsql = "SELECT name FROM PRAGMA_TABLE_INFO('project_"+projectId+"_households')"
    psql = "SELECT name FROM PRAGMA_TABLE_INFO('project_"+projectId+"_persons')"
...
def viewBySelectedVar(projectId, selectedVar):
    ...
    # varNames = selectedVar.split("_")
   
    # type = varNames[0]
    # selectedVariable = varNames[1]
    type = selectedVar[0]
    selectedVariable = selectedVar[2:]

    print("varNames", selectedVar, type, selectedVariable)
...
def countsByProjectId(projectId):
    ...
    persons= conn.execute(
        psql ).fetchall()  
    print("total households and persons", households[0].values(), persons[0].values())                  
...
def deleteByProjectId(projectId):
    ...
    cur.execute('''DELETE from projects_users where project_id = ?''', (projectId,))
...
def projectCreate_1():
    ...
    file_names = ["controls","geo_cross_walk",  "control_totals_tract", "control_totals_bg","seed_households", "seed_persons","settings"]
    ...
    #4 - write output files from json data to the matching folder
        ...
        if i == 0:
            ...
            complete_path = os.path.join(path,subfolder_name, file_names[0]+".csv") 
        ...
        # do yaml here 
            ...
            complete_path_yaml = os.path.join(path,subfolder_name, file_names[6]+".yaml") 
        ...
        #data folder      
            ...
            path1 = os.path.join(path,subfolder_name, file_names[1]+".csv")
            path2 = os.path.join(path,subfolder_name, file_names[2]+".csv")
            path3 = os.path.join(path,subfolder_name, file_names[3]+".csv")
            path4 = os.path.join(path,subfolder_name, file_names[4]+".csv")
            path5 = os.path.join(path,subfolder_name, file_names[5]+".csv")
        
        
            complete_path = [path1,path2,path3,path4, path5]
            data_data  = [crosswork_data, control_tracts,control_bgs, seed_household, seed_person]
...
#   create a task for RQ
    ...
    add_run_args(parser)
    args = parser.parse_args(['--working_dir', os.getcwd() + '/popsynth_runs/' + directory])
